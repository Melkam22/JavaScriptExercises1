...
function longestWord (input5) {
    ...
    for(let b=0; b<givenWord.length; b++) {
        if(givenWord[b].length > biggerOne) {
            biggerOne = givenWord[b].length; 
            myWord = givenWord[b];//with length we ll get the figure without the word
        }   
    }
    console.log(myWord);     
}
...
eachLetterOccurrence("communication");//it ll count each character 
